#!/usr/bin/env bash
#
# Wrapper for the k3s + VirtualBox scenario.
#
...
#
# Env passthrough: RESOURCE_PROFILE, TENANT, NETWORK_MODE, PROVIDER,
# VBOX_GUEST_DISK, RHEL_USERNAME, RHEL_PASSWORD all reach vagrant.
...
# Pre-parse named flags from argv (allowed anywhere). Strip them so the
# remaining positional indices stay backwards-compatible.
...
# macOS bash 3.2 + set -u: expanding an empty array with "${arr[@]}" errors.
# Use the ${arr[@]+...} guard so empty arrays produce zero args, not an error.
