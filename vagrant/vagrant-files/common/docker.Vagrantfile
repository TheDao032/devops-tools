...
RESOURCES = {
  "coordinator" => {
    ...
    # "ram" => [ram_selector * 512, 2048].max(),
    "cpu" => CPU_CORES >= 12 ? 2 : 1,
    # "swap" => "128m",
  },
  "worker" => {
    # "ram" => [ram_selector * 172, 2048].min(),
    "cpu" => (((CPU_CORES / 4) * 4) - 4) / 2,
    # "swap" => "128m",
  },
  "connection-pool" => {
    # "ram" => [ram_selector * 172, 2048].min(),
...
def get_bridge_adapter()
  if OS.windows?
    ...
    return %x{configuration/networks/#{VAGRANT_PROVIDER}/macos/macos-bridge.sh}.chomp
  end
end
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "coordinator" do |node|
    ...
    node.vm.provider "docker" do |docker|
      ...
      docker.create_args = [
        "--cpus=#{RESOURCES["coordinator"]["cpu"]}",
        # "--memory=#{RESOURCES["coordinator"]["ram"]}g",
        # "--memory-swap=#{RESOURCES["coordinator"]["swap"]}",
      ]
  ...
  (1..NUM_SLAVE_CLUSTERS).each do |i|
    config.vm.define "worker-#{i}" do |node|
      ...
      node.vm.provider "docker" do |docker|
        ...
        docker.create_args = [
          "--cpus=#{RESOURCES["worker"]["cpu"]}",
          # "--memory=#{RESOURCES["worker"]["ram"]}",
          # "--memory-swap=#{RESOURCES["worker"]["swap"]}",
        ]
  ...
  config.vm.define "connection-pool" do |node|
    ...
    node.vm.provider "docker" do |docker|
      ...
      docker.create_args = [
        "--cpus=#{RESOURCES["connection-pool"]["cpu"]}",
        # "--memory=#{RESOURCES["connection-pool"]["ram"]}",
        # "--memory-swap=#{RESOURCES["connection-pool"]["swap"]}",
      ]
