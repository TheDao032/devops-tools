...
require_relative '../../containers/vmware_fusion/ubuntu'
...
cpu_per_master = (total_master_cpu / (NUM_AGENTS >= 1 ? NUM_AGENTS : 1)).to_i
...
ram_per_worker = (remaining_ram / (NUM_AGENTS >= 1 ? NUM_AGENTS : 1)).to_i
cpu_per_worker = (remaining_cpu / (NUM_AGENTS >= 1 ? NUM_AGENTS : 1)).to_i
...
machines.push(
  {
    name: "ubuntu",
    box: vmWareFusionConfig.os_systems[:ubuntu][:box],
    os: vmWareFusionConfig.os_systems[:ubuntu][:os],
    cpu: RESOURCES[:server][:cpu],
    ram: RESOURCES[:server][:ram],
    network: {
      name: "",
      ports: [
        { guest: 22, host: 2790 + 1 }
      ],
      ip: "#{IP_NW}.#{SERVER_IP_START + 1}"
    },
    files: [
      { source: "./configuration/os/#{vmWareFusionConfig.os_systems[:ubuntu][:os]}/.tmux.conf", destination: "$HOME/.tmux.conf" },
      { source: "./configuration/os/#{vmWareFusionConfig.os_systems[:ubuntu][:os]}/.vimrc", destination: "$HOME/.vimrc" }
    ]
  }
)
...
Vagrant.configure("2") do |config|
  ...
  machines.each do |machine|
    ...
    vm.define(
      ...
      ip_nw = IP_NW,
      machines = machines,
      os_system_info = vmWareFusionConfig.os_systems[:ubuntu]
    )
  end
