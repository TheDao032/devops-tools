...
INVENTORY=${ANSIBLE_INVENTORIES_DIR}/${ENVIRONMENT}/${SERVICE}/${PROVIDER}
...
# LIB_FILE=${DEVOPS_TOOLS_DIR}/deployments/ansible/${ENVIRONMENT}/env-vars/k3s-env.bash
# source "${LIB_FILE}" || { log_info "$(date -u) - FATAL - failure occured while reading ${LIB_FILE}"; exit 1; }
...
ansible_exec() {
  uv run ansible/inventories/${ENVIRONMENT}/${SERVICE}/${PROVIDER}/dynamic_inventory.py --list
