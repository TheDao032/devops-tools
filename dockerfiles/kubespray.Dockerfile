...
# Create an SSH user
RUN groupadd appadm && \
		useradd -rm --create-home -s /bin/bash -g root -G sudo appadm && \
		usermod -aG sudo appadm && \
    echo "appadm:${APPADM_PASS}" | chpasswd

# Allow SSH access
RUN mkdir -p /home/appadm/.ssh && \
    chmod 700 /home/appadm/.ssh && \
    touch /home/appadm/.ssh/authorized_keys && \
    chmod 600 /home/appadm/.ssh/authorized_keys && \
    chown -R appadm /home/appadm/.ssh
...
RUN echo 'appadm ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/appadm && \
    chmod 0440 /etc/sudoers.d/appadm
