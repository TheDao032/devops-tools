...
def generate_inventory():
    groups = {
        'all': {
            ...
            'vars': {
                'psql_version': '15',
                'citus_version': '11.3',
                'server_ip': '172.29.139.102',
                'server_cidr_range': '172.29.139.0/24',
                'docker_network_cidr_range': '172.20.10.0/24',
                'docker_default_cidr_range': '172.17.0.0/16',
            }
        },
    ...
    for vm, ip in ips.items():
        if ip:
            if "coordinator" in vm:
                groups['coordinator']['hosts'].append(vm)
            elif "worker" in vm:
                groups['worker']['hosts'].append(vm)
