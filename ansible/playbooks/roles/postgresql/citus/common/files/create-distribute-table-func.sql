...
BEGIN
    ...
    LOOP
        ...
        table_identity := obj.object_identity;

        schema_name := split_part(table_identity, '.', 1);
        table_name := split_part(table_identity, '.', 2);
        ...
        RAISE NOTICE 'Table %.% has been created.', schema_name, table_name;
...
CREATE EVENT TRIGGER distribute_on_create_table
ON ddl_command_end
WHEN TAG IN ('CREATE TABLE')
EXECUTE FUNCTION auto_distribute_table();
